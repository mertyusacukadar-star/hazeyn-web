...
function settingsOf(state) {
  ...
  return {
    ...
    address: text(settings.address) || 'Ümraniye / İstanbul',
    ...
    mapUrl: safeUrl(settings.googleMapsEmbedUrl || settings.mapsEmbedUrl),
  ...
  return `<header class="site-header seo-site-header" id="top">
    <nav class="nav container" aria-label="Ana menü">
      ...
      <div class="nav-actions" id="navActions">
        <a class="btn btn-ghost phone-link" data-track="phone_call" href="${escapeHtml(phoneHref(settings.phone))}">Hemen Ara</a>
        <a class="btn btn-gold whatsapp-link" data-track="whatsapp_click" href="${escapeHtml(whatsappHref(settings, 'Merhaba, Hazeyn Turizm web sitenizden ulaşıyorum. Güncel Umre programları hakkında bilgi almak istiyorum.'))}" target="_blank" rel="noopener">WhatsApp</a>
        <a class="btn btn-ghost instagram-link" href="${escapeHtml(instagram)}" target="_blank" rel="noopener">Instagram</a>
      </div>
    </nav>
  </header>`;
}
...
const PAGE_CSS = `<style>
  .seo-page{background:var(--bg)}.seo-page main{overflow:hidden}.seo-skip{position:fixed;left:12px;top:-60px;z-index:100;background:#fff;padding:10px 14px;border-radius:10px}.seo-skip:focus{top:12px}
  .seo-hero{position:relative;min-height:540px;display:flex;align-items:flex-end;color:#fff;background:#111;padding:150px 0 72px}.seo-hero-media{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}.seo-hero:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(5,5,8,.92),rgba(8,8,10,.58) 58%,rgba(8,8,10,.2))}.seo-hero-copy{position:relative;z-index:1;max-width:820px}.seo-hero h1{font-size:clamp(42px,6vw,76px);letter-spacing:-.05em;line-height:1.02;margin:0 0 18px}.seo-hero p{font-size:clamp(17px,2vw,23px);color:rgba(255,255,255,.84);max-width:720px;margin:0}.seo-breadcrumb{display:flex;gap:8px;flex-wrap:wrap;font-size:13px;margin-bottom:18px;color:rgba(255,255,255,.75)}.seo-breadcrumb a{color:var(--gold2)}
  ...
  .seo-hotel-card{padding:0;overflow:hidden}.seo-hotel-card>div{padding:26px}.seo-hotel-card b{display:block;color:var(--gold);margin-top:12px}.seo-price-card{display:flex;flex-direction:column;gap:12px;min-height:190px}.seo-price-card strong{font-size:26px;margin-top:auto}.seo-price-card.featured{border-color:rgba(196,145,47,.55);background:linear-gradient(145deg,#fff,#fff8e7)}.seo-check-list{list-style:none;padding:0;margin:0;display:grid;gap:10px}.seo-check-list li{position:relative;padding-left:25px}.seo-check-list li:before{content:"✓";position:absolute;left:0;color:var(--gold);font-weight:900}.seo-check-list.excluded li:before{content:"–"}.seo-prose{font-size:17px;color:#38332d}.seo-prose p{margin:0 0 18px}.seo-gallery{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.seo-gallery figure{margin:0;border-radius:20px;overflow:hidden;background:#111;min-height:240px}.seo-gallery img{width:100%;height:100%;min-height:240px;object-fit:cover}.seo-expired{display:flex;align-items:center;justify-content:space-between;gap:20px;background:#241d15;color:#fff;border-radius:22px;padding:24px 28px;margin:24px 0}.seo-expired strong{font-size:22px}.seo-expired p{margin:4px 0 0;color:rgba(255,255,255,.72)}
...
<html lang="tr"><head>
  ...
  <link rel="icon" type="image/png" sizes="32x32" href="/assets/icon.png"><link rel="apple-touch-icon" href="/assets/icon.png"><link rel="stylesheet" href="/style.css?v=20260811-growth4">${jsonLd}
...
function renderProgramPage(state, tourInput, origin) {
  ...
  const schemas = [organizationSchema(state, origin), breadcrumbSchema(origin, [{ name: 'Ana Sayfa', path: '/tr' }, { name: 'Umre Programları', path: '/tr#umre' }, { name: tour.title, path: canonicalPath }]), {
    '@context': 'https://schema.org', '@type': 'TouristTrip', name: tour.title, description: tour.seoDescription, image: tour.groupImages.map((image) => absoluteUrl(image, origin)), touristType: 'Umre yolcusu', itinerary: tour.visitProgram || tour.program,
  ...
  const gallery = tour.groupImages.filter(Boolean).map((image, index) => `<figure><img src="${escapeHtml(image)}" alt="${escapeHtml(`${tour.title} ${index === 0 ? 'program görseli' : `otel ve kafile görseli ${index + 1}`}`)}" loading="lazy" decoding="async"></figure>`).join('');
  const content = `<section class="seo-hero"><img class="seo-hero-media" src="${escapeHtml(tour.image)}" alt="${escapeHtml(`${tour.title} kapak görseli`)}" fetchpriority="high"><div class="container seo-hero-copy">
    <div class="seo-breadcrumb"><a href="/tr">Ana Sayfa</a><span>/</span><a href="/tr#umre">Umre Programları</a><span>/</span><span>${escapeHtml(tour.title)}</span></div><span class="eyebrow">${escapeHtml(tour.capacityStatus)}</span><h1>${escapeHtml(tour.title)}</h1><p>${escapeHtml(formatDate(tour.departureDate))} · ${escapeHtml(tour.durationLabel)} · ${escapeHtml(cities)} çıkışlı</p>
    ...
    ${expiredPanel}
    <section class="seo-section"><div class="seo-section-head"><span class="section-kicker">Konaklama ve ulaşım</span><h2>Mekke ve Medine otelleri</h2></div><div class="seo-grid-2"><article class="seo-card seo-hotel-card"><div><span class="section-kicker">Mekke</span><h3>${escapeHtml(tour.mekkeHotelName)}</h3><b>Harem’e mesafe / servis</b><p>${escapeHtml(tour.mekkeDistanceService)}</p></div></article><article class="seo-card seo-hotel-card"><div><span class="section-kicker">Medine</span><h3>${escapeHtml(tour.medineHotelName)}</h3><b>Mescid-i Nebevî’ye mesafe / servis</b><p>${escapeHtml(tour.medineDistanceService)}</p></div></article></div><article class="seo-card" style="margin-top:20px"><span class="section-kicker">Uçuş</span><h3>Havayolu ve uçuş bilgileri</h3><p>${escapeHtml(tour.flightDetails)}</p></article></section>
    ...
    <section class="seo-section"><div class="seo-grid-2"><article class="seo-card seo-prose"><span class="section-kicker">Ziyaretler</span><h2>Ziyaret programı</h2>${paragraphs(tour.visitProgram || 'Mekke ve Medine ziyaretleri kafile rehberi eşliğinde programlanır.')}</article><article class="seo-card seo-prose"><span class="section-kicker">Gün gün akış</span><h2>Program ayrıntıları</h2>${paragraphs(tour.program)}</article></div></section>
    ${gallery ? `<section class="seo-section"><div class="seo-section-head"><span class="section-kicker">Görseller</span><h2>Otel ve kafile görselleri</h2></div><div class="seo-gallery">${gallery}</div></section>` : ''}
  </div>`;
  return basePage({ state, origin, title: tour.seoTitle, description: tour.seoDescription, canonicalPath, ogImage: tour.image, content, schemas, pageType: 'program', pageTitle: tour.title, pageId: tour.id, pageSlug: tour.slug });
}
...
function renderLocalPage(state, origin) {
  ...
  const mapUrl = settings.mapUrl || `https://www.google.com/maps?q=${encodeURIComponent(settings.address)}&output=embed`;
